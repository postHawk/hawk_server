...
void encrypt(encrypt_param * e_param)
{
	...
	if(len > BUFF_SIZE)
	{
		printf("use long buffer\n");
		int b_len = BUFF_SIZE + EVP_CIPHER_CTX_block_size(ctx) - 1;
		printf("long buffer len %i\n", b_len);
		unsigned char long_out_buffer[b_len];
		...
		while(i < b_len)
		{
			if(out_buffer[i])
			{
				long_out_buffer[i] = out_buffer[i];
			}
			else
			{
				long_out_buffer[i] = 0;
		...
		printf("data long buffer %s\n", (char *)long_out_buffer);
		
		res = EVP_EncryptFinal_ex(ctx, long_out_buffer, &len);
		e_param->result.append((char *) long_out_buffer, 0, len);
	}
	else
	{
		printf("use normal buffer\n");
		res = EVP_EncryptFinal_ex(ctx, out_buffer, &len);
		e_param->result.append((char *) out_buffer, 0, len);
	}
...
void decrypt(encrypt_param * e_param)
{
	...
	if(len > BUFF_SIZE)
	{
		...
		printf("data long buffer %s\n", (char *)long_out_buffer);
		res = EVP_DecryptFinal_ex(ctx, long_out_buffer, &len);
		e_param->result.append((char *) long_out_buffer, 0, len);
	}
	else
	{
		printf("use normal buffer\n");
		res = EVP_DecryptFinal_ex(ctx, out_buffer, &len);
		e_param->result.append((char *) out_buffer, 0, len);
	}
