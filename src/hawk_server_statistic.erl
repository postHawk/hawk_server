...
handle_call({new_message_from_host, Host}, _From, #state{count_all_message=Cnt_mess, ets_table=TableId} = State) ->
	...
	{ok, Connection} = mc_worker_api:connect (?DB_NAME),
	...
	mc_worker_api:update(Connection, <<"message_log">>, {<<"domain">>, Host, <<"time">>, Time}, Command, [{upsert, true}]),
...
init_message(Host) ->
	...
	case User of
		...
		_ ->
			...
			{true,{result,Res}} = mc_worker_api:command(Connection, {aggregate, <<"message_log">>, pipeline, [
					  {'$match', {domain, B_host}},
